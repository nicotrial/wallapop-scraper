#!/usr/bin/env python3
"""
Wallapop Search Scraper v2 using Playwright

Uses a headless browser with request interception to capture API responses.
"""
...
class WallapopScraper:
                ...
                if 'search' in response.url or 'catalog' in response.url:
                    if response.status == 200:
                        ...
                        if 'json' in content_type:
                            ...
                            print(f"  Captured API response from: {response.url[:60]}...")
            except Exception:
                pass
    ...
    def _scroll_for_results(self, page, captured_data: List[Dict], max_results: int):
    ...
    def _parse_api_data(self, captured_data: List[Dict], max_results: int) -> List[SearchResult]:
        ...
        for data in captured_data:
            ...
            if 'search_objects' in data:
                ...
                if 'search' in data['data']:
                    items = data['data']['search'].get('items', [])
                elif 'items' in data['data']:
                    items = data['data']['items']
                elif 'section' in data['data'] and isinstance(data['data']['section'], dict):
                    items = data['data']['section'].get('items', [])

            if not items:
                continue

            for item in self._format_items(items):
            ...
            if elements:
                ...
                for el in elements[:max_results]:
                    try:
                        ...
                        title = el.query_selector('h2, .title, [class*="title"]')
                        price = el.query_selector('.price, [class*="price"]')
                        ...
                        if title:
                            items.append(SearchResult(
                                ...
                                description='',
                                price=self._parse_price(price.inner_text() if price else '0'),
                                currency='EUR',
                                city='',
                                image_url=None,
                                url=f"https://es.wallapop.com{link}" if link and link.startswith('/') else link,
                                seller=''
                            ))
                    except Exception:
                        pass
    ...
    def _format_items(self, items: List[Dict]) -> List[SearchResult]:
        """Format API items to SearchResult objects."""
        results = []
        for item in items:
    ...
    def _count_captured_items(self, captured_data: List[Dict]) -> int:
        """Count unique items across captured API responses."""
        seen_ids = set()
        for data in captured_data:
            for item in self._parse_api_data([data], max_results=1000):
                if item.id:
                    seen_ids.add(item.id)
        return len(seen_ids)
